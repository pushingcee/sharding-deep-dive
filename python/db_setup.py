...
def generate_data_from_csv(args: Namespace) -> None:
    ...
    if not generator.data_exists():
        logger.error(f"CSV data not found at '{DATA_DIR}'. Run with --generate-csv first.")
    ...
    try:
        ...
        logger.error(f"Database error during CSV load: {error}")
        sys.exit(EXIT_FAILURE)
    except Exception as e:
        logger.error(f"Unexpected error during CSV load: {e}")
        sys.exit(EXIT_FAILURE)
